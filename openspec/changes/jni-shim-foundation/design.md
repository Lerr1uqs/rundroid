...
Emulator facade
        ...
        ├── Python shim registration surface
        │       │ 只声明/收集 metadata
        │       ▼
        │   bindings/python registration bridge
        │
        ├── Rust builtin registration surface
        │       │ macro / builder / builtin class declarations
        │       ▼
        │   Rust class registration bridge
        │
        ▼
AndroidRuntime
        │ normalize to unified class definition
        ▼
emulator/jni runtime authority
        │
        ├── object / refs
        ├── method / field sig
        ├── dispatch
...
- Rust builtin registration surface 也不直接持有最终权威状态
- Python / Rust 都只是注册入口，最终统一进入 `AndroidRuntime`
- `PyEmulator` 不应继续作为 class/member/object 最终状态容器
...
这类 Python binding 状态若短期保留，也只能是 adapter cache：

- 不得成为最终 class/member/object identity 来源
- 不得替代 `AndroidRuntime` / `AndroidVM` / `RefTable`
- 后续应优先收敛到 runtime-owned id + binding adapter 的结构
