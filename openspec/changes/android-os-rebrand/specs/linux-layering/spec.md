...
> 本次 delta 跟随 `linux` → `android` 品牌重命名：`linux crate` → `android crate`、`LinuxRuntime` → `Kernel`，并把 syscall ABI 边界的承载主体从"`syscall` 模块上的 `impl LinuxRuntime`"对齐为"`Syscall` 类型上的 `impl Syscall`"（见 `android-syscall-surface`）。分层核心契约（kernel 产数据不碰 `MemoryBridge` / syscall 经单一 `MemoryBridge` 回写 `EFAULT` / errno 集中 / 域分文件 / 语义可单测）语义不变。

### Requirement: Kernel owns OS state and semantics without touching target memory

android crate SHALL 把 OS 状态（VFS 挂载表、fd 表、mmap 区游标、brk、stdout、PRNG 种子、设备注册表）与纯语义方法收敛到 `kernel` 模块（`Kernel` 聚合根 + 各域分文件 impl），且这些方法 SHALL 只产出数据（如 `Vec<u8>` / `usize` / `u64`）或推进 OS 状态，不接收 `MemoryBridge`。`MemoryBridge` 是 syscall 边界注入的 guest 内存访问抽象，底层通常落到 engine / `GuestCPU::mem_read` / `mem_write` / `mem_map`，不是 Python 专属概念。
...
android crate 的 kernel 模块 SHALL 按 OS 子系统域（fd IO / 内存管理 / 随机数 / 路径解析 / 设备注册）分文件组织 OS 语义方法，而不是单文件堆叠。`Kernel` 作为聚合根，各域方法以分文件 `impl Kernel`（同 crate 跨文件 impl）实现，对调用方只暴露聚合方法。`stdout/stderr` 收集 SHALL 归属 kernel 的 `write` 语义，而不是 syscall 层单独分支。
...
#### Scenario: Kernel aggregates subsystem domains

- **WHEN** syscall 层或测试访问 OS 语义
- **THEN** 它 SHALL 通过 `Kernel` 聚合根调用（如 `kernel.read_at` / `kernel.alloc_mmap_addr` / `kernel.getrandom_bytes`）
...
- **THEN** 它 SHALL 把字节追加到 `Kernel.stdout`
- **AND** syscall 层不 SHALL 再保留独立的 stdout/stderr 写分支

### Requirement: Syscall layer carries ABI boundary and target-side writeback

android crate 的 syscall 层 SHALL 限定为 ABI 边界，承载于独立的 `Syscall` 类型（`impl Syscall`，由 `Kernel` 通过 `syscalls: Syscall` 字段持有，见 `android-syscall-surface`）：`Syscall::dispatch` 解码寄存器参数、调用 `Kernel` 的 OS 方法、通过 `MemoryBridge` 把结果回写到目标侧内存、编码 `SyscallResult`。回写失败 SHALL 上抛 `EFAULT`。

#### Scenario: Handler writes back through target-side closures

- **WHEN** `sys_*` handler（在 `impl Syscall` 上）从 `Kernel` OS 方法拿到数据
...
- **THEN** `Syscall::dispatch`（经 `Kernel::dispatch` 转发）SHALL 按 syscall 号解码参数并路由到对应 handler
...
android crate SHALL 定义单一 `MemoryBridge` trait 作为 syscall 层访问 guest 内存的唯一抽象，至少包含 `read` / `write` / `map` 三个方法；`Syscall::dispatch` 与各 `sys_*` handler SHALL 接收该 trait，而不是三个独立闭包或完整 `GuestCPU`。
...
android crate SHALL 通过 `errno.rs` 集中定义 errno 常量与 kernel/底座错误到 errno 的映射，避免在 `Syscall` 各 handler 内分散硬编码错误码选择。
...
### Requirement: Refactor preserves observable behavior of implemented syscalls

OS 层重构 SHALL 保持**已实现** syscall 的对外可观察行为（返回值、errno、目标侧可见状态、fd 生命周期、VFS 解析）不变。**未实现** syscall 的行为变化（静默 `ENOSYS` → 显式 `Unimplemented` + 上层策略）由 `android-syscall-surface` 契约管辖，不在本要求范围。

#### Scenario: Existing syscall tests and cases stay green

- **WHEN** 现有 syscall 单测（含 pread64 四测）与 case `01/02/03/04` 在重构后运行
- **THEN** 它们 SHALL 全部继续通过
- **AND** 已实现 syscall 的返回值 / errno / 目标侧可见状态 SHALL 与重构前一致
...
- **THEN** 它 SHALL 能直接调用 `Kernel` 方法（如 `kernel.read_at(fd, offset, count)`）
- **AND** 不 SHALL 需要构造 `Syscall::dispatch(SYS_PREAD64, ...)` + mock `MemoryBridge`
