# python-jni-value-marshalling Specification

## Purpose
定义 Python 值与 JNI 值（`JValue` / `JniArgs`）跨 FFI 边界的双向编组契约：Python 侧 `str`/`bytes`/`bool`/`int`/`float`/`None` 稳定转换入 JNI，Rust 侧 `JValue`（Object / Null / primitive / String / byte[]）回编组到对应 Python 表示；编组失败 fail-fast（不静默吞成 `None`）；`str`→String、`bytes`→byte[] 提供默认身份 coercion，`JavaString`/`JavaByteArray` 作为可选显式身份层用于复用对象身份。确保 `Signature([B)V`、对象回传、返回值校验等基础 JNI 用例自然成立且不回退到 null 占位。
## Requirements
