# linux-layering Specification

## Purpose
定义 `rundroid-linux` crate 内 **kernel（OS 状态与语义）** 与 **syscall（ABI 边界与目标侧回写）** 的职责分离边界：kernel 方法只产出数据 / 推进 OS 状态、不接收 `MemoryBridge`；syscall 通过单一 `MemoryBridge`（read / write / map）完成目标侧回写，回写失败上抛 `EFAULT`；errno 常量与底座错误→errno 映射集中到 `errno.rs`。目的是在 syscall 集持续扩张（socket / procfs / signal / ...）时，让 OS 语义可脱离 syscall 号 / 寄存器 / mock bridge 独立单测，并让 "source → 目标侧回写 → 返回值" 主线有清晰落点、不漂移。
## Requirements
