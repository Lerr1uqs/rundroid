# android-framework-stubs Specification

## Purpose
定义 Android framework stubs 的 class-oriented registry 模型：用 class-spec 驱动的 `FrameworkRegistry` 作为 builtin framework class 的声明式 catalog（含 service registry 与 APK-backed `PackageManager` / `PackageInfo` / `Signature` / `AssetManager` / `Bundle` 等最小 stub 集）。约束 Rust builtin framework class 与 Python shim override 共用同一套 class/member 数据模型与同一 dispatch 主线，区别只在注册入口与优先级，避免退回 unidbg `AbstractJni` 式 giant switch 维护陷阱。
## Requirements
