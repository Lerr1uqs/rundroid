...
│   ├── os/linux/             # LinuxRuntime / syscall.rs(ABI边界,待拆) / fd.rs / vfs.rs  ← linux-syscall-layering
│   ├── jni/                  # JNI 子系统（见下"核心概念"）
│   ├── case-runner/          # GuestRuntime(装配层) / jni_hook(trampoline) / case / manifest / runtime
...
├── tests/cases/              # case.toml 数据：01-pure-export / 02-openat / 03-dev-urandom / 04-mmap-rw
...
os/linux ◄───────────────────────────── # LinuxRuntime(syscall/fd/vfs)
  │
jni ◄─────────────────────────────────── # AndroidRuntime/VM/registry/JNIEnvABI
  │
...
| `AndroidRuntime`/`AndroidVM` | jni/android_runtime.rs,android_vm.rs | JNI 聚合根（registry/objects/refs/exceptions/natives/apk），AndroidRuntime 用 RwLock |
...
| `JniTrampolineHook` | case-runner/jni_hook.rs | CodeHook：trampoline 拦截 → function_index 反算 slot → dispatch_jni_call → JniEnvSurface |
...
2. **syscall svc hook + 目标侧回写**：guest `svc #0` → SyscallHook → `LinuxRuntime::dispatch(nr,x0..x5, read_guest/write_guest/map_guest)` → handler 调 OS 语义拿数据 → **write_guest/map_guest 回写失败即 EFAULT**（不允许"返回长度但目标缓冲没变"的假成功）。
3. **ELF 装载链**：parser → loader（footprint 整块 RWX reserve → 写段 → 按 p_flags mem_protect 收紧）→ linker（relocation 写回 → resolve 按依赖图 → RELRO mem_protect 只读）→ `JNI_OnLoad` 自动检测调用。
...
5. **Python↔Rust**：pyo3 cdylib `_rundroid` + Python `javashim`（`JavaClass`/`JavaObject`/`avm`）。`avm` 是 JNI/VM 唯一门面；marshalling 单一规则源（`py_to_jvalue`/`jvalue_to_py`）；guest JNI 经 trampoline hook 回调 Python override。死锁规避：锁内不进 Python、AndroidRuntime 用 RwLock。
...
- **死锁**：jni_hook 持 `Arc<Mutex<AndroidVM>>`，锁内**不进 Python**；`AndroidRuntime` 用 `RwLock`（`register_java_object` 取 `&self`）。
...
5. `emulator/case-runner/src/jni_hook.rs` — trampoline 拦截 → dispatch 链路
